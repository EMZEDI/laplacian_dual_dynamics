...
from src.nets import MLP, generate_hk_module_fn
from src.tools import saving, timer_tools
...
def main(hyperparams):
    ...
    np.random.seed(
        hparam_yaml['seed']
    )  # TODO: Check if this is the best way to set the seed
    ...
    # Create trainer
    d = hparam_yaml['d']
    jax.random.PRNGKey(hparam_yaml['seed'])
    hidden_dims = hparam_yaml['hidden_dims']
    env_name = hparam_yaml['env_name']
    date_time = hparam_yaml['date_time']

    generate_hk_module_fn(MLP, d, hidden_dims, hparam_yaml['activation'])
    ...
    # Create environment
    create_env(
        env_name, 'LapGrid-v0', hparam_yaml['seed'], hparam_yaml['max_episode_steps']
    )
